...
###########################################################################################################################################################
		...
		rndDwnCurveLength = math.trunc(mc.arclen(curveObj))
		if rndDwnCurveLength <= 1:
			print "This is why it's a good idea to animate the emitter to 0, several frames before the animation ends. Otherwise it may emit a particle at the end of the timeline and there are less then 2 spans in the curve at the end."
		else:
